...
final class Alg_WC_Currency_Switcher {
	...
	function __construct() {
		...
		// The filter
		add_filter( 'alg_wc_currency_switcher_plugin_option', array( $this, 'currency_switcher_plugin_option' ), PHP_INT_MAX, 2 );
	 ...
	 * @since   1.0.0
	 */
	function currency_switcher_plugin_option( $option, $type = 'value_currencies' ) {
		switch ( $type ) {
			case 'settings':
				return '';
			case 'value_flags':
				return get_option( 'alg_wc_currency_switcher_flags_enabled', 'no' );
			default: // 'value_currencies'
				return PHP_INT_MAX;
		}
	}
